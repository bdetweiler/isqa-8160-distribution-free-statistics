...
```{r}
penetration <- c(5.37, 5.39, 5.42, 5.51, 5.63, 5.74, 5.82, 5.83, 5.94, 5.98, 6.07, 6.07, 6.13, 6.20, 6.21, 6.23, 6.25, 6.26, 6.26, 6.28, 6.29, 6.31, 6.35, 6.41, 6.57, 6.67, 6.81, 7.03, 7.40, 7.44, 7.82, 8.03, 8.11, 8.44, 8.51, 8.72, 8.83, 9.04, 9.33, 9.51, 9.61, 9.68, 9.82, 10, 10, 10, 10, 10, 10, 10)
...
```{r}
q <- function(n, r, m, percentile) {
  (4*n - 2*(r + m - 1) - qchisq(p=percentile, df=2*(r + m)))/(4*n - 2*(r + m - 1) + qchisq(p=percentile, df=2*(r + m)))
}
qchisq(p=0.90, df=2*(0 + 1))
q(n=86, r=0, m=1, percentile=0.90)
```
...
q(n=86, r=0, m=1, percentile=0.95)
```
